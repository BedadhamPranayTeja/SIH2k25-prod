...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = await JudgingCriteria.find().sort({ round: 1, order: 1, name: 1 });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const created = await JudgingCriteria.create({ ...data, isActive: data.isActive ?? true, order: data.order ?? 0 });
